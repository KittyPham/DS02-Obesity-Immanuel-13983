...
# Preprocessing Data
    ...
    if X.isnull().sum().sum() > 0 or np.any(np.isinf(X)):
        st.error("Error: X contains NaN or infinite values.")
        st.write(X.isnull().sum())
        st.stop()

    if y.isnull().sum() > 0:
        st.error("Error: y contains NaN values.")
        st.stop()
    ...
    - Data dibagi menjadi 80% train dan 20% test tanpa resampling SMOTE.
    """)
...
# Pemodelan & Evaluasi
    ...
    - Perhatikan bahwa tanpa SMOTE, performa model mungkin dipengaruhi oleh ketidakseimbangan kelas.
    """)
